...
import { sunatInboxService, type SunatInboxListRow } from '../../services/sunatInbox';
...
const SunatInboxListPage = ({ workspace }: SunatInboxListPageProps) => {
  ...
  return (
    <div className={PAGE_WORKSPACE_CLASS}>
      <div>
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Buzón SOL SUNAT</h1>
        <p className="text-slate-500 mt-1 text-sm">
          Gestión manual de comunicados y evidencias por empresa y período. Sin integración con SUNAT.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3 bg-white rounded-xl border border-slate-200 p-3 shadow-sm">
        ...
        <div className="min-w-[10rem]">
          ...
          <select
            ...
            className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm outline-none focus:ring-2 focus:ring-primary-500"
          >
            {SUNAT_INBOX_STATUS_FILTER.map((opt) => (
              ...
              {loading && rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row.company_id} className="hover:bg-slate-50/80">
                    ...
                    <td className={TD}>
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${sunatInboxStatusBadgeClass(row.status)}`}
                      >
                        {sunatInboxStatusLabel(row.status)}
                      </span>
                    </td>
